...
echo "${INFO} Changed directory to ${NOTE_INFO}$(pwd)"
    python3 -m dfss --url=open@sophgo.com:sophon-demo/Qwen-VL/qwen-vl-int8-vit-fp16-1dev.bmodel
popd > /dev/null
...
if [ ! -d "../models/token_config" ];
then
pushd ../models > /dev/null
echo "${INFO} Changed directory to ${NOTE_INFO}$(pwd)"
    python3 -m dfss --url=open@sophgo.com:sophon-demo/Qwen-VL/token_config.zip
